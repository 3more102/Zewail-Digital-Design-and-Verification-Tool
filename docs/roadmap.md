...
retained as unsupported evidence and bidirectional waveform cross-probing across trusted
direct bindings, assertion-to-waveform correlation, and the
...
- [x] Bidirectional waveform cross-probing across normalized parent-signal/child-pin bindings; child-port direction is used only from normalized module-port evidence.
