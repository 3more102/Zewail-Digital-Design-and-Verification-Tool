...
Phase/objection lifecycle normalization, explicit sequence report-context evidence, normalized sequence state lifecycle ingestion, normalized sequence-item handshake persistence/history, explicit item log-marker ingestion, and a generated UVM sequence-item instrumentation wrapper are implemented; automatic sequence-state/vendor adapters and arbitration priority/fairness reconstruction remain planned.
...
- [x] Portable generated UVM sequence-item instrumentation wrapper emitting explicit `ZDDV_UVM_ITEM` evidence.\n- [ ] Automatic sequence-state/vendor instrumentation adapters and arbitration priority/fairness reconstruction.
