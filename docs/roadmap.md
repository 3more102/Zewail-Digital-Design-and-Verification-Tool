...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Captured XML is structurally inventoried and fingerprinted without assigning undocumented item semantics; detailed code-coverage item/source normalization and holes remain planned.
...
- [x] Questa detailed XML structure inventory and schema fingerprint.
