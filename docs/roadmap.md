...
item-level UCDB normalization/holes remain planned; phase/objection lifecycle normalization and the normalized UVM sequence lifecycle result model are implemented; sequence-item/arbitration reconstruction remains planned.
...
- [x] Sequence-aware UVM lifecycle ingestion through an explicit normalized JSON contract.
- [ ] Sequence-item/arbitration lifecycle reconstruction where supported.
