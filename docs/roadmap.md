...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Statement and branch detail rows are normalized into source-linked coverage holes; condition/expression/toggle/FSM item-level normalization remains planned.
