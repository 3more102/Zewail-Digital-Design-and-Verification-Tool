...
Current status: deterministic source indexing, source-level hierarchy, source-level
driver/load navigation, VCD waveform indexing, assertion-to-waveform correlation,
and the first protocol transaction reconstruction path (normalized APB traces) are
implemented. Elaborated hierarchy and waveform-driven protocol extraction remain planned.

- [x] Design hierarchy (source-level).
- [x] Source indexing.
- [x] Waveform index (VCD scopes/signals; FST artifact metadata).
- [x] Drivers/loads navigation (conservative source-level references).
