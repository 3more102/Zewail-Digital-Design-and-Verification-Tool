...
indexing and cross-probing, assertion-to-waveform correlation, and the first protocol
transaction reconstruction path (normalized APB traces) are implemented. Elaborated
hierarchy, drivers/loads navigation, and waveform-driven protocol extraction remain planned.

- [x] Design hierarchy (source-level).
- [x] Source indexing.
- [x] Waveform index (VCD scopes/signals; FST artifact metadata).
- [x] Waveform cross-probing (VCD exact-tick values and bounded transition windows).
