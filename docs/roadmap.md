...
- [x] Waveform navigation and bounded in-memory targeted VCD probe integration.
- [x] Detailed assertion/UVM/formal evidence panes with bounded persisted-evidence queries.
- [x] Review-gated lint/build/run actions using existing core APIs, exact request SHA-256 confirmation, and no arbitrary shell command surface.
