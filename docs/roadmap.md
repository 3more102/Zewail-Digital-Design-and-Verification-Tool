...
Current status: evidence/source/hierarchy/waveform views reuse the authoritative
ZDDV backend and remain read-only. Explicit desktop actions are review-gated:
lint/build/run require an exact SHA-256 over the reviewed action/project state, while
generated verification artifacts use isolated staging plus exact-content
SHA-confirmed apply. The desktop does not invoke AI or automatically execute/apply
reviewed operations.
