...
item-level UCDB normalization/holes remain planned; phase/objection lifecycle normalization is implemented, and explicit `sequencer@@sequence` report-context evidence is normalized without inferring sequence start/end semantics.
...
- [x] Sequence report-evidence ingestion when explicit `sequencer@@sequence` context is present.
