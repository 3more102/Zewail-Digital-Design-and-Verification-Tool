...
Phase/objection lifecycle normalization, explicit sequence report-context evidence, normalized sequence state lifecycle ingestion, and normalized sequence-item handshake persistence/history are implemented. The Xcelium foundation now also instruments native coverage, retains per-run `.ucm`/`.ucd` databases, merges them with IMC, and stores explicitly labeled Overall Average/Covered summary scores in percentage-native history; deeper IMC metric/item normalization remains future work. Automatic sequence/item instrumentation adapters and arbitration priority/fairness reconstruction remain planned.
...
- [x] Xcelium native coverage capture, IMC merge, and Overall Average/Covered summary normalization.
