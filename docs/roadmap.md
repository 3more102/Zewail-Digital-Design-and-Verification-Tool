...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Statement/branch detail rows plus scalar condition/expression FEC rows are normalized into the shared coverage-hole model; multibit condition/expression FEC layouts and toggle/FSM itemization remain planned.
