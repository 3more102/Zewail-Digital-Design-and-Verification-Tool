...
functional-coverage database. Machine-readable XML detail output, complementary zero-hit source/file-line evidence, multibit-expression evidence, and dedicated by-instance toggle detail are retained when available. Documented statement/branch detail, scalar condition/expression FEC rows, and FSM state/transition rows are normalized into the shared coverage-hole model; multibit condition/expression and toggle item normalization remain planned.
...
- [x] Questa dedicated by-instance toggle detail evidence capture.
...
- [x] Xcelium Overall Average/Covered metric normalization into percentage-native ZDDV coverage history.
