...
child-pin query evidence retained separately, and direction-qualified boundary
driver/load roles, assertion-to-waveform correlation, and the first protocol transaction
reconstruction path (normalized APB traces) are implemented. Exact internal elaborated
...
- [x] Direction-qualified boundary driver/load evidence for direct normalized pin bindings, with unavailable direction kept unclassified.
