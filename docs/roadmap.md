...
functional-coverage database. A machine-readable detailed XML report and a complementary zero-hit source/file-line text report are also retained when available; detailed code-coverage item/source normalization and holes remain planned.
...
- [x] Questa zero-hit source/file-line coverage evidence export.
