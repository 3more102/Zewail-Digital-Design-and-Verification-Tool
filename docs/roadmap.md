...
Detailed covergroup bins are also normalized into the existing functional-coverage database;
code-coverage item/source-hole normalization remains planned. Phase/objection lifecycle normalization is implemented, while sequence-aware UVM semantics remain planned.
...
- [ ] Questa code-coverage item/source-hole normalization.
...
- [ ] Sequence-aware UVM result ingestion where supported.
