...
functional-coverage database. A machine-readable detailed XML report is also retained when available. Statement and branch items from `vcover report -details -code sb` are normalized into the shared code-hole model; condition/expression/toggle/FSM itemization remains planned.
...
- [x] Questa statement/branch code-coverage item/source normalization and coverage-hole reporting.
- [ ] Questa condition/expression/toggle/FSM detailed item normalization.
- [x] VCS execution adapter foundation (build/run, deterministic seed/test/plusargs, VCD, assertions, run-linked UVM).
