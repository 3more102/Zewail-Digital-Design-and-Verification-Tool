...
Phase/objection lifecycle normalization, explicit sequence report-context evidence, normalized sequence state lifecycle ingestion, observed UVM item grant-order reconstruction, and normalized sequence-item handshake history/violation persistence are implemented; automatic sequence instrumentation and deeper arbitration/fairness reconstruction remain planned.
...
- [x] Normalized sequence-item handshake analysis with observed grant-order evidence, SQLite event/violation persistence, and history queries.
