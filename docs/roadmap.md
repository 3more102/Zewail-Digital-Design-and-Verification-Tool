...
functional-coverage database. A machine-readable detailed XML report is also retained when available; statement items are normalized from that evidence into source-located zero-hit hole reports, while the remaining code-coverage item types remain planned.
