...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Detailed statement and branch report rows are normalized into source-aware code points and zero-hit coverage-hole reports; condition/expression/toggle/FSM item normalization remains planned.
...
- [x] Questa statement/branch item-level normalization and coverage-hole reporting.
