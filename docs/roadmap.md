...
Current status: refresh/navigation is read-only and reuses the authoritative ZDDV
verification database. Explicit project actions are separately review-gated: the GUI
hashes the exact structured lint/build/run request, binds it to active project identity,
and executes only after the user re-enters the complete SHA-256. No arbitrary shell
...
- [x] Review-gated structured lint/build/run actions using existing core APIs, active-project identity binding, and exact SHA-256 confirmation.
