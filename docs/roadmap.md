...
`vcover report -summary` normalization into ZDDV coverage history, ordinary covergroup-bin ingestion, and retained XML coverage evidence are implemented; code item-level XML normalization/holes remain planned. Phase/objection lifecycle normalization and explicit sequence report-context evidence are implemented, while sequence start/end semantics remain planned.
...
- [x] Questa ordinary functional covergroup-bin ingestion.
- [x] Questa XML coverage evidence retention.
- [ ] Questa code item-level XML normalization and coverage-hole reporting.
