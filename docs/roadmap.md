...
functional-coverage database. A machine-readable detailed XML report is also retained when available. Statement records are normalized to instance, source-file, line, statement-index, and hit-count evidence for zero-hit hole reporting; remaining code-item types remain planned.
