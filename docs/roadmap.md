...
functional-coverage database. Machine-readable XML detail output, complementary zero-hit source/file-line evidence, and multibit-expression evidence are retained when available. Documented statement and branch detail, scalar condition/expression FEC rows, FSM state/transition rows, and documented multibit-expression operand-bit FEC terms are normalized into the shared coverage-hole model; multibit condition and toggle itemization remain planned.
