...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Documented statement/branch plus scalar condition/expression FEC detail rows are normalized into the shared coverage-hole model; multibit condition/expression and toggle/FSM itemization remain planned.
