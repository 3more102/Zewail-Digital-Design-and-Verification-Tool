...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Documented statement and branch detail rows plus condition/expression FEC rows are normalized into the shared coverage-hole model; alternate FEC layouts and toggle/FSM itemization remain planned.
