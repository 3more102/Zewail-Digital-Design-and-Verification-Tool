...
normalized pin bindings and direct instance-boundary driver/load classification from
normalized port directions, assertion-to-waveform correlation, and the first protocol
transaction reconstruction path (normalized APB traces) are implemented. Deeper
intra-module elaborated net/process drivers and loads remain future enrichment work.
...
- [x] Direct elaborated instance-boundary driver/load classification from normalized pin bindings plus normalized port-direction evidence, with unknown directions left unclassified.
