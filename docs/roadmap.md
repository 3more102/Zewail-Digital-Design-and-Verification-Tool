...
Current status: persisted evidence views remain read-only and reuse the authoritative
ZDDV verification database. The Review Actions tab can stage generated verification
proposals and apply exact reviewed bytes only through the existing SHA-confirmed core
gate. The GUI still does not execute simulations, invoke AI, or infer verification state.
