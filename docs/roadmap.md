...
Phase/objection lifecycle normalization, explicit sequence report-context evidence, normalized sequence state lifecycle ingestion, normalized sequence-item handshake persistence/history, and explicit sequencer arbitration evidence are implemented; automatic simulator instrumentation/adapters remain planned.
...
- [x] Explicit UVM sequencer arbitration REQUEST/GRANT analysis with FIFO/STRICT policy checks, optional max-bypass evidence policy, SQLite persistence, and history filtering.\n- [ ] Automatic sequence/item/arbitration instrumentation and simulator-specific adapters.
