...
Phase/objection lifecycle normalization, explicit sequence report-context evidence, normalized sequence state lifecycle ingestion, normalized sequence-item handshake persistence/history, observed grant-order reconstruction, and explicit simulator-log marker extraction are implemented; automatic UVM-side instrumentation injection, vendor-log adapters, and policy-aware arbitration/fairness remain planned.
...
- [x] Observed sequence-item grant-order reconstruction from explicit GRANT evidence.\n- [x] Explicit `ZDDV_UVM_ITEM` simulator-log marker extraction with source-line provenance and run correlation.\n- [ ] Automatic UVM-side sequence/item instrumentation injection and vendor-log adapters.\n- [ ] Policy-aware arbitration/fairness analysis from explicit request/grant evidence.
