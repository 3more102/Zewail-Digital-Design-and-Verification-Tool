...
functional-coverage database. A machine-readable detailed XML report is also retained when available, and ZDDV records a namespace-insensitive XML structure inventory plus schema fingerprint without assigning undocumented item semantics; detailed code-coverage item/source normalization and holes remain planned.
