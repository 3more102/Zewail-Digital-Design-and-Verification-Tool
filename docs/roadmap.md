...
optional USER-sideband transport evidence with explicit per-signal width/presence metadata, explicit ID_W_WIDTH/ID_R_WIDTH transaction-ID presence and range evidence plus conservative complete-pair VCD width capture, data-width-aware AxSIZE/WSTRB byte-lane
