...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Documented branch detail rows are normalized into code coverage-hole reporting; other detailed code item classes remain planned.
...
- [x] Questa branch code-coverage item/source normalization and coverage-hole reporting.
