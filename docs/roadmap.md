...
functional-coverage database. Machine-readable XML detail output and complementary zero-hit source/file-line evidence are retained when available. Source-linked statement, branch, condition, and expression zero-hit items are normalized only when their per-type counts reconcile with summary misses; toggle/FSM item-level mapping remains planned.
...
- [x] Questa source-linked statement/branch/condition/expression zero-hit normalization and coverage-hole reporting.
- [ ] Questa toggle/FSM item-level hole normalization.
