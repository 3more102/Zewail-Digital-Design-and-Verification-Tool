...
`vcover report -summary` normalization into ZDDV coverage history are implemented, and detailed ordinary covergroup
bins are ingested into the simulator-independent functional-coverage database;
code-coverage item-level UCDB normalization/holes remain planned; phase/objection lifecycle normalization and explicit sequence report-context evidence are implemented, while sequence start/end semantics remain planned.
