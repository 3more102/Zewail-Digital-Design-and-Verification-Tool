...
Current status: evidence, source, hierarchy, and waveform browsing remain read-only and
reuse the authoritative ZDDV verification database. Explicit Actions may execute
lint/build/run only after exact SHA-256 review and live project/source revalidation.
Generated Review may stage or apply exact reviewed assertion/test bytes through the
existing generated-artifact core gate. The desktop does not invoke AI.
...
- [x] Separate generated-artifact review pane with bounded preview, integrity re-hash, exact SHA-256 confirmation, and existing stage/apply core APIs.
