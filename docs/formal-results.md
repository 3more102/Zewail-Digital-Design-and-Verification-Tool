...
## Formal trace source cross-probing

A normalized counterexample or witness can be mapped back to source hierarchy,
SystemVerilog declarations, and source-structural driver/load evidence with:

```text
zddv --project <project> formal-trace-crossprobe <normalized-trace.json>
zddv --project <project> formal-trace-crossprobe <normalized-trace.json> --signal state
```

This consumes the normalized trace signal catalog only; it does not infer a semantic root
cause from value changes. Signal matching delegates to the existing Debug Studio
cross-probe engine so waveform and formal navigation use the same conservative source model.
...
formats, SQLite trace-sample persistence, semantic failure-cause analysis, and broader
