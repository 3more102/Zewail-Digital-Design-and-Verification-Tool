...
status/return code, the engine summary when present, explicit failed assertion
names, explicit reached cover statements, and explicitly reported VCD trace
paths. It does not synthesize PASS rows for properties that the logfile never
enumerates, and it does not convert a bounded PASS into an unbounded proof.
