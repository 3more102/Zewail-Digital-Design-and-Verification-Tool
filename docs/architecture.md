...
sources, so hierarchy captured before an RTL/config revision is rejected as
stale. Valid simulator-resolved evidence, including generated-scope paths, is
preferred before deterministic source-hierarchy fallback. Invalid or stale
persisted elaboration is reported as evidence state and is never guessed or
