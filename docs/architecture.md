...
models the pending request queue and configured UVM arbitration mode. ZDDV checks FIFO,
STRICT_FIFO, and STRICT_RANDOM rules deterministically; RANDOM, WEIGHTED, and USER selections
remain observational. An optional `max_bypass` threshold is a ZDDV project policy rather than a
UVM-standard starvation guarantee. Arbitration events, decisions, violations, summaries, and run
