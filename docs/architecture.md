...
- **Cadence Xcelium foundation**: `xrun -elaborate` builds a reusable simulator database under the project build directory and `xrun -R -xmlibdirname <db>` executes that snapshot from isolated run directories. The adapter transports deterministic SystemVerilog seeds with `-svseed`, preserves ZDDV/UVM test plusargs, enforces external timeouts, ingests assertion markers and UVM logs into the shared run database, and can generate VCD evidence with an `-input` Tcl script using Xcelium `database`/`probe` commands. When coverage is enabled, elaboration uses `-coverage all`; each simulation selects an isolated Cadence workdir/scope/test hierarchy and ZDDV records the native run directory only when a `.ucd` file is present. The coverage engine merges captured run directories with Cadence IMC in batch mode and retains the merged native database, summary report, exact command script, and manifest as evidence. For the documented cumulative summary layout, ZDDV preserves Overall/Code/FSM/Functional Average and Covered grades separately, uses Overall Covered as the percentage-native history score, and persists only explicitly reported covered/total counts. Unknown layouts remain evidence-only rather than being guessed.
