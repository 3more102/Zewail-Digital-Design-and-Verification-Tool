...
`zddv crossprobe` composes existing normalized evidence instead of introducing
simulator-specific debug logic. A query resolves one indexed waveform signal,
maps its scope to the source-level hierarchy, resolves the matching design unit
and declaration when possible, and attaches source-level structural driver/load
evidence for the same signal.

Matching is intentionally conservative. Exact paths are preferred, suffix paths
must resolve uniquely, and ambiguous short signal names are rejected. The report
retains the design, waveform, and connectivity index paths used as evidence so a
future GUI can implement deterministic source/waveform cross-navigation.
