...
- **Cadence Xcelium foundation**: `xrun -elaborate` builds a reusable simulator database under the project build directory and `xrun -R -xmlibdirname <db>` executes that snapshot from isolated run directories. The adapter transports deterministic SystemVerilog seeds with `-svseed`, preserves ZDDV/UVM test plusargs, enforces external timeouts, ingests assertion markers and UVM logs into the shared run database, and can generate VCD evidence with an `-input` Tcl script using Xcelium `database`/`probe` commands. When coverage is enabled, elaboration uses `-coverage all`; each simulation selects an isolated Cadence workdir/scope/test hierarchy and ZDDV records the native run directory only when a `.ucd` file is present. The coverage engine can merge captured run directories with Cadence IMC in batch mode and retains the generated merged native database, cumulative all-metrics summary report, exact command script, and manifest as evidence. When the retained summary exposes the labelled Overall/Code/FSM/Functional Average/Covered columns, ZDDV preserves each numeric grade separately and persists `Overall Covered` as the percentage-native coverage-history score. Parenthesized item-count payloads remain raw evidence rather than being assigned inferred field semantics; unknown summary layouts remain evidence-only.
