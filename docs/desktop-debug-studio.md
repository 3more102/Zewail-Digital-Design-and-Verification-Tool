...
Waveform navigation/targeted VCD probing is already integrated through the existing
core APIs. The remaining desktop milestone is review-gated project actions, which
must continue to reuse the existing core APIs rather than duplicate simulator-specific
logic in the GUI.
