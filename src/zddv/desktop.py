...
def launch_desktop_gui(
    ...
    *,
    ...
    ttk.Label(
        footer,
        text=(
            "Display-only viewer: refresh reads persisted evidence and project-owned "
            "source files; it does not run verification, invoke AI, write design "
            "indexes, or apply generated artifacts."
        ),
    ).pack(side="left")
    ...
    def _show_source(path_value: str, line: int | None = None) -> None:
        try:
            source_value = _read_source(project, path_value)
        ...
        source_text.insert("1.0", source_value)
        source_text.configure(state="disabled")
