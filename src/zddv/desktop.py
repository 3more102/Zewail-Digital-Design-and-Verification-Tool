...
def _load_persisted_elaborated_hierarchy(project: ProjectConfig) -> dict[str, Any]:
    ...
    empty_ports = {"ports": [], "port_evidence": {"status": "NOT_PRESENT"}}
    if not path.exists():
        return {
            "status": "NOT_PRESENT",
            "path": str(path),
    ...
    if not isinstance(payload, dict):
        return {
            "status": "INVALID",
            "path": str(path),
            "error": "elaborated hierarchy payload must be a JSON object",
            "instances": [],
            "ports": [],
            "port_evidence": {"status": "INVALID"},
        }
...
def launch_desktop_gui(
    ...
    *,
    ...
    elaborated_tree.heading("#0", text="Hierarchy path / port")
    ...
    def refresh() -> None:
        ...
        if elaborated["status"] == "PRESENT":
            ...
            if (elaborated.get("port_evidence") or {}).get("status") == "NORMALIZED":
            ...
            for module_ports in ports_by_module.values():
                module_ports.sort(
                    key=lambda item: (
