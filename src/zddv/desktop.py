...
def _resolve_configured_source(project: ProjectConfig, value: str) -> Path:
    """Resolve a source path only when it belongs to the configured project sources."""
    candidate = Path(value)
    if not candidate.is_absolute():
        candidate = project.root / candidate
    candidate = candidate.resolve()

    allowed = {path.resolve() for path in project.source_files()}
    if candidate not in allowed:
        raise ValueError(
            f"Source preview is restricted to configured project sources: {value}"
        )
    if not candidate.is_file():
        raise FileNotFoundError(f"Configured source file was not found: {candidate}")
    return candidate


def _read_source(project: ProjectConfig, value: str) -> str:
    """Read one configured project source for display without mutating it."""
    path = _resolve_configured_source(project, value)
...
def launch_desktop_gui(
    ...
    *,
    ...
    source_title_text = tk.StringVar(value="Select a configured source file or design unit.")
    ...
    ttk.Label(
        footer,
        text=(
            "Display-only viewer: refresh reads persisted evidence and configured source files; "
