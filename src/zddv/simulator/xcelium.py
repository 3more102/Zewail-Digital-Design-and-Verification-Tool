...
class XceliumBackend(SimulatorBackend):
    """Cadence Xcelium adapter using xrun elaborate/re-run flow."""
    ...
    def version(self) -> str:
        ...
        for command in ([tool, "-version"], [tool, "-64bit", "-version"]):
        ...
        raise RuntimeError(
            diagnostics[-1]
            if diagnostics
            else "Unable to query Cadence Xcelium version."
        )
    ...
    def build(self, project: ProjectConfig) -> BuildResult:
        ...
        command.extend(["-elaborate", *[str(path) for path in sources]])
        ...
        coverage_capture = (
            "pending-native-support" if project.coverage else "disabled"
        )
        manifest = {
            "simulator": self.name,
    ...
    def _write_input_file(self, run_dir: Path) -> Path:
    ...
    def run(
        ...
        *,
        ...
        if project.waveform:
            input_file = self._write_input_file(run_dir)
        ...
        record = {
            ...
            "input_script": str(input_file) if input_file else None,
        }
