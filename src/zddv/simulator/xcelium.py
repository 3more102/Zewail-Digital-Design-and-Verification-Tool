...
class XceliumBackend(SimulatorBackend):
    """Cadence Xcelium adapter using xrun elaborate + snapshot reuse."""
    ...
    def build(self, project: ProjectConfig) -> BuildResult:
        ...
        coverage_capture = "not-implemented" if project.coverage else "disabled"
        manifest = {
            "simulator": self.name,
    ...
    def _write_input_file(self, run_dir: Path, *, waveform: bool) -> Path:
    ...
    def run(
        ...
        *,
        ...
        input_file = self._write_input_file(run_dir, waveform=project.waveform)
