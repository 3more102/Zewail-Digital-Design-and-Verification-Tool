...
def _parse_sequence_event(event: dict[str, Any]) -> dict[str, Any] | None:
    ...
    sequencer, sequence_name = component.split("@@", 1)
    sequencer = sequencer.strip()
    sequence_name = sequence_name.strip()
    if not sequencer or not sequence_name:
        return None
...
def _parse_lifecycle(messages: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    sequences_seen = list(dict.fromkeys(item["sequence"] for item in sequence_events))
...
def parse_uvm_log_text(text: str, *, source: str = "uvm-log") -> dict[str, Any]:
    ...
    return {
        ...
        "limitations": [
            ...
            "Sequence report-context evidence is normalized when a UVM component path explicitly contains sequencer@@sequence; this does not infer sequence start/end semantics.",
