...
def _parse_lifecycle(messages: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    sequences_seen = list(
        dict.fromkeys(item["sequence"] for item in sequence_events)
    )
...
def parse_uvm_log_text(text: str, *, source: str = "uvm-log") -> dict[str, Any]:
    ...
    return {
        ...
        "limitations": [
            ...
            "Sequence report-context evidence is recorded only when a report component explicitly contains sequencer@@sequence; this does not claim sequence start/end lifecycle semantics.",
