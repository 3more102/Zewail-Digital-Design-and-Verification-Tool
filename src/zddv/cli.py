...
def cmd_generated_stage(args) -> int:
    project = load_project(_project_arg(args))
    result = stage_generated_artifact(
        project,
        args.path,
        output_root=args.output_root,
    )
    print(
        f"GENERATED DRAFT: kind={result['kind']} name={result['name']} "
        f"status={result['status']}"
    )
    print(f"SHA-256: {result['content_sha256']}")
    print(f"Draft: {result['content_path']}")
    print(f"Manifest: {result['manifest_path']}")
    if result.get("suggested_target_path"):
        print(f"Suggested target: {result['suggested_target_path']}")
    print("Project source modified: no")
    print("Execution: disabled")
    return 0


def cmd_generated_apply(args) -> int:
    project = load_project(_project_arg(args))
    ...
    print(
        f"GENERATED ARTIFACT APPLIED: kind={result['kind']} "
        f"name={result['name']} status={result['status']}"
    )
    print(f"SHA-256: {result['content_sha256']}")
    print(f"Destination: {result['destination']}")
    print("Execution: disabled")
    print(f"Record: {result['applied_record_path']}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_assertion_waveform.add_argument(
        ...
        metavar="PATH",
        help=(
            "Explicitly allow FST-to-VCD conversion for assertion correlation; "
            "optionally provide the fst2vcd executable path"
        ),
    )
    ...
    p_generated_stage = sub.add_parser(
        "generated-stage",
        help="Stage proposed assertion/test code for review without modifying project sources",
    )
    p_generated_stage.add_argument(
        "path",
        help="Generation proposal JSON inside the project root",
    )
    p_generated_stage.add_argument(
        "--output-root",
        default=".zddv/generated",
        help="Review-isolated staging root; must remain under .zddv/generated",
    )
    p_generated_stage.set_defaults(func=cmd_generated_stage)

    p_generated_apply = sub.add_parser(
        "generated-apply",
        help="Apply exact reviewed generated bytes after explicit SHA-confirmed approval",
    )
    p_generated_apply.add_argument(
        "manifest",
        help="Staged generated-artifact manifest JSON",
    )
    p_generated_apply.add_argument(
        "--destination",
        default=None,
        help="Project-relative .sv/.svh destination; defaults to the proposal target",
    )
    p_generated_apply.add_argument(
        "--expected-sha256",
        required=True,
        help="Exact SHA-256 of the reviewed staged content",
    )
    p_generated_apply.add_argument(
        "--approve-reviewed",
        action="store_true",
        help="Explicit opt-in confirming the staged bytes were reviewed",
    )
    p_generated_apply.set_defaults(func=cmd_generated_apply)
