...
def cmd_uvm_item_history(args) -> int:
    ...
    print(
        ...
        f"{'G/R/D/RSP':<15} {'ACTIVE':>6} {'PARTIAL':>7} "
        f"{'RUN':<24} SNAPSHOT"
    )
    for row in rows:
        run_id = row["run_id"] or "-"
        handshake = (
            f"{row['granted_count']}/{row['requested_count']}/"
            f"{row['completed_count']}/{row['responded_count']}"
        )
        print(
            f"{row['status']:<6} {row['item_count']:>5} "
            f"{row['event_count']:>6} {row['violation_count']:>5} "
            f"{handshake:<15} {row['active_count']:>6} "
            f"{row['partial_count']:>7} {run_id[:24]:<24} "
            f"{row['snapshot_id']}"
        )
    return 0
