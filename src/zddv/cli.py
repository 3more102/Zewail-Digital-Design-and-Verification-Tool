...
def cmd_coverage(args) -> int:
    ...
    if item_metrics is not None:
        print(
            "Normalized item coverage: "
            f"{item_metrics['hit_points']}/{item_metrics['total_points']} hit "
            f"({item_metrics['hit_rate']:.1f}%)"
        )
        if result.get("points_path"):
            print(f"Normalized points: {result['points_path']}")
    elif result.get("item_capture_status") not in (None, "ok"):
        print(
            "Normalized item coverage: unavailable "
            f"({result['item_capture_status']})"
        )
    if result.get("code_report"):
        print(f"Detailed code report: {result['code_report']}")
