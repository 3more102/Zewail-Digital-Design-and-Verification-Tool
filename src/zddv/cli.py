...
def cmd_formal_history(args) -> int:
    ...
    print(
        f"{'STATUS':<8} {'MODE':<6} {'PROPS':>5} {'CEX':>4} "
        f"{'SAFE':>5} {'PROVED':>6} {'COV':>4} {'BACKEND':<16} SNAPSHOT"
    )
    for row in rows:
        print(
            f"{row['status']:<8} {row['mode']:<6} {row['property_count']:>5} "
            f"{row['counterexample_count']:>4} {row['bounded_safe_count']:>5} "
            f"{row['proved_count']:>6} {row['covered_goal_count']:>4} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_formal_import = sub.add_parser(
        "formal-import",
        help="Import normalized formal result evidence into JSON and SQLite history",
    )
    ...
    p_formal_history = sub.add_parser(
        "formal-history",
        help="Show persisted normalized formal result history",
    )
