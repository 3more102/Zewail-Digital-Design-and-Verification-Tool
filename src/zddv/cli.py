...
def cmd_coverage(args) -> int:
    ...
    if detailed:
        ...
        branch_detail = detailed.get("branch_detail", {})
        if branch_detail:
            print(
                "Branch detail: "
                f"{branch_detail.get('status', 'unknown')} "
                f"{branch_detail.get('path', '-')}"
            )
            if result.get("branch_detail_points") is not None:
                print(
                    "Branch items: "
                    f"{result.get('branch_detail_points', 0)} total, "
                    f"{result.get('branch_detail_holes', 0)} uncovered"
                )
...
def cmd_coverage_holes(args) -> int:
    ...
    if simulator == "verilator":
        ...
        if args.point_type not in {None, "branch"}:
            raise RuntimeError(
                "Questa code coverage-hole normalization currently supports "
                "branch items only."
            )
        report_path = (
            project.root / ".zddv" / "coverage" / "branch-details.txt"
        ).resolve()
        if not report_path.exists():
            raise RuntimeError(
                f"Detailed Questa branch report not found at {report_path}. "
                "Run 'zddv coverage' first."
            )
        points = parse_questa_branch_coverage_report(
    ...
    if not points:
        raise RuntimeError(f"No normalized coverage points found in {source_label}.")
