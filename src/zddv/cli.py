...
def cmd_coverage_holes(args) -> int:
    ...
    if simulator == "verilator":
        ...
        report = write_questa_statement_hole_report(project, output, limit=args.limit)
    else:
        raise RuntimeError(
            f"Coverage-hole reporting is not implemented for simulator: {project.simulator}"
        )
    ...
    if report.get("xml"):
        print(f"Detailed coverage XML: {report['xml']}")
    if report.get("zeros"):
        print(f"Zero-detail evidence: {report['zeros']}")
