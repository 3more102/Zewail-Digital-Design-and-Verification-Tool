...
def cmd_uvm_sequence_history(args) -> int:
    project = load_project(_project_arg(args))
    rows = list_uvm_sequence_snapshots(
        project,
        limit=args.limit,
        status=args.status,
        run_id=args.run_id,
    ...
    if not rows:
        print("No UVM sequence snapshots found.")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_uvm_sequence = sub.add_parser(
        ...
        help="Analyze normalized UVM sequence lifecycle events from JSON",
    )
    p_uvm_sequence.add_argument(
        "path",
        help="Normalized UVM sequence lifecycle event JSON file",
    )
    p_uvm_sequence.add_argument(
        "--run",
        dest="run_id",
    ...
    p_uvm_sequence_history = sub.add_parser(
        ...
        help="Show persisted UVM sequence lifecycle snapshots",
    )
    ...
    p_uvm_sequence_history.add_argument(
        ...
        dest="run_id",
        default=None,
        help="Filter sequence snapshots linked to a recorded ZDDV run ID",
    )
