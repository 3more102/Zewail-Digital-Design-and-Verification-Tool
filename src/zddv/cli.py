...
def cmd_coverage(args) -> int:
    ...
    normalized_code = result.get("normalized_code_coverage") or {}
    if normalized_code:
        print(
            "Normalized code coverage holes: "
            f"{normalized_code.get('status', 'unknown')} "
            f"{normalized_code.get('hole_items', 0)} item(s) "
            f"{normalized_code.get('path', '-')}"
        )
...
def cmd_coverage_holes(args) -> int:
    ...
    if simulator == "verilator":
        ...
        if status != "ok":
            ...
            detail = f" See {evidence_path}." if evidence_path else ""
            raise RuntimeError(
    ...
    if not points:
        print("Coverage holes: 0 unhit point(s)")
        return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_coverage_holes.add_argument(
        ...
        default=None,
        help=(
            "Optional normalized coverage point type filter: line/toggle for "
            "Verilator; statement/branch/condition/expression/toggle/fsm for Questa"
        ),
    )
    p_coverage_holes.add_argument(
        "--limit",
