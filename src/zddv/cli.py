...
def cmd_uvm_item_history(args) -> int:
    ...
    print(
        ...
        f"{'G/R/D/RSP':<13} {'ACTIVE/PART':<11} {'RUN':<24} SNAPSHOT"
    )
    for row in rows:
        handshake = (
            f"{row['granted_count']}/"
        ...
        print(
            ...
            f"{handshake:<13} {activity:<11} {run_id[:24]:<24} "
            f"{row['snapshot_id']}"
        )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_uvm_item_history.add_argument(
        ...
        dest="run_id",
        default=None,
        help="Filter UVM item handshake snapshots linked to a recorded ZDDV run ID",
    )
