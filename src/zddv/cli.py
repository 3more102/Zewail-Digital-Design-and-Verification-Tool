...
def cmd_axi4_analyze(args) -> int:
    ...
    print(
        f"Read/Write bursts: "
        f"{summary['issued_read_bursts']}/{summary['issued_write_bursts']}  "
        f"Error responses: {summary['error_responses']}"
    )
    stalls = summary["channel_stall_cycles"]
    print(
    ...
    for violation in result["violations"][: args.show]:
        channel = (
            f"[{violation.get('channel')}] "
            if violation.get("channel")
            else ""
        )
        print(
    ...
    if len(result["violations"]) > args.show:
        print(
            f"... {len(result['violations']) - args.show} "
            "more violation(s)"
        )
    print(f"Report: {result['report_path']}")
    return 0 if result["status"] == "PASS" else 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_axi4 = sub.add_parser(
        "axi4-analyze",
        help="Reconstruct AXI4 bursts and report protocol violations",
    )
