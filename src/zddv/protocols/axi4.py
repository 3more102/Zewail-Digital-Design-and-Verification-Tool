...
def analyze_axi4_trace(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    def make_request(sample: dict[str, Any], prefix: str, index: int) -> dict[str, Any]:
        ...
        if (
            ...
            add_violation(
                "transfer_size_exceeds_data_bus", sample,
                f"{prefix}SIZE selects {beat_bytes} bytes on a {data_bus_bytes}-byte data bus",
                channel=prefix, signal=f"{prefix}SIZE",
                expected=f"transfer size <= {data_bus_bytes} bytes", actual=beat_bytes,
            )
    ...
    def validate_write_strobes(
        ...
        if not isinstance(beat_bytes, int) or beat_bytes <= 0 or beat_bytes > data_bus_bytes:
            return
        ...
        for beat_index, (beat, beat_addr) in enumerate(zip(beats, addresses)):
            ...
            if upper_lane < lower_lane:
                beat["allowed_strb_mask"] = None
                continue
            allowed_mask = ((1 << (upper_lane - lower_lane + 1)) - 1) << lower_lane
        ...
        if b_hs:
            ...
            if not queue:
                ...
                tx = {
                    ...
                    ] if data_bus_bytes is not None else None,
                    "response": label,
                    "response_code": code,
    ...
    result = {
        ...
        "limitations": [
            ...
            "When data_width_bits is known, AxSIZE is bounded by the data bus and WSTRB is checked against the legal byte lanes for each write beat.",
