...
CREATE TABLE IF NOT EXISTS uvm_sequence_snapshots (
    snapshot_id TEXT PRIMARY KEY,
    created_at TEXT NOT NULL,
    project TEXT NOT NULL,
    source TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_uvm_sequence_snapshots_created
    ON uvm_sequence_snapshots(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_uvm_sequence_snapshots_status
    ON uvm_sequence_snapshots(status);

CREATE INDEX IF NOT EXISTS idx_uvm_sequence_snapshots_run
    ON uvm_sequence_snapshots(run_id);
...
CREATE INDEX IF NOT EXISTS idx_uvm_sequence_events_sequence
    ON uvm_sequence_events(snapshot_id, sequence_id);

CREATE INDEX IF NOT EXISTS idx_uvm_sequence_events_state
    ON uvm_sequence_events(state);
...
def record_uvm_sequence_snapshot(
    project: ProjectConfig,
    record: dict[str, Any],
) -> Path:
    path = database_path(project)
    ...
    with _connect(project) as db:
        db.execute(
            """
            INSERT OR REPLACE INTO uvm_sequence_snapshots (
...
def list_uvm_sequence_snapshots(
    project: ProjectConfig,
    *,
    limit: int = 20,
    status: str | None = None,
