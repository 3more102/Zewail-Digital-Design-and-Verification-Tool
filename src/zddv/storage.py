    ...
    with _connect(project) as db:
        ...
        db.executemany(
            ...
            INSERT INTO uvm_sequence_events (
                snapshot_id, event_index, sequencer_name, sequence_name,
                action, time_text, report_id, message_event_index, log_line, raw
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            [
                (
...
def list_uvm_sequence_events(
    ...
    with _connect(project) as db:
        rows = db.execute(
            """
            SELECT snapshot_id, event_index, sequencer_name, sequence_name,
                   action, time_text, report_id, message_event_index,
                   log_line, raw
