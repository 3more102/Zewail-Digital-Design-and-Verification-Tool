...
CREATE INDEX IF NOT EXISTS idx_uvm_item_events_item
    ON uvm_item_handshake_events(snapshot_id, item_id);

CREATE INDEX IF NOT EXISTS idx_uvm_item_events_event
    ...
    with _connect(project) as db:
        db.execute(
            ...
            INSERT OR REPLACE INTO uvm_item_handshake_snapshots (
                ...
                granted_count, requested_count, completed_count, responded_count,
                active_count, partial_count, run_id,
                input_path, normalized_path, report_path
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
