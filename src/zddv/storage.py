...
CREATE TABLE IF NOT EXISTS formal_result_snapshots (
    ...
    request_timeout_s REAL,
    request_properties_json TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_formal_results_created_at
    ON formal_result_snapshots(created_at DESC);

CREATE INDEX IF NOT EXISTS idx_formal_results_status
    ON formal_result_snapshots(status);

CREATE INDEX IF NOT EXISTS idx_formal_results_mode
...
def record_formal_result_snapshot(
    ...
    """Persist one normalized formal result and its property evidence."""
    path = database_path(project)
    summary = record["summary"]
    ...
    with _connect(project) as db:
        db.execute(
            ...
            INSERT OR REPLACE INTO formal_result_snapshots (
                ...
                mode, proof_scope, request_depth, request_timeout_s,
                request_properties_json, property_count, assertion_count,
                cover_count, counterexample_count, bounded_safe_count,
                proved_count, covered_goal_count, unreached_goal_count,
                returncode, runtime_ms, run_dir, log_path, command_json,
                artifacts_json, input_path, report_path
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                record["snapshot_id"],
                ...
                json.dumps(request.get("properties", []), sort_keys=True),
        ...
        db.executemany(
            ...
            [
                (
                    ...
                    item.get("message"),
                    (item.get("trace") or {}).get("path"),
                    (item.get("trace") or {}).get("role"),
                )
...
def list_formal_result_snapshots(
    ...
    *,
    ...
    if mode is not None and mode not in {"bmc", "prove", "cover"}:
    ...
    for row in rows:
        item = dict(row)
        item["request_properties"] = json.loads(item.pop("request_properties_json"))
