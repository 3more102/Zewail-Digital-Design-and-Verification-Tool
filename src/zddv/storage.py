...
CREATE INDEX IF NOT EXISTS idx_formal_results_backend
...
def record_formal_result_snapshot(
    ...
    """Persist one normalized formal evidence snapshot and its property rows."""
    path = database_path(project)
    summary = record["summary"]
    ...
    with _connect(project) as db:
        db.execute(
            ...
            INSERT OR REPLACE INTO formal_result_snapshots (
                ...
                mode, scope, depth, property_count, assertion_count,
                cover_count, counterexample_count, bounded_safe_count,
                proved_count, covered_goal_count, unreached_goal_count,
                input_path, report_path, run_dir, log_path, returncode,
                runtime_ms, command_json, artifacts_json
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                record["snapshot_id"],
        ...
        db.executemany(
            ...
            [
                ...
                for index, item in enumerate(record.get("properties", []))
            ],
        )
    return path
...
def list_formal_result_snapshots(
    ...
    *,
    ...
    """List persisted formal evidence snapshots without reinterpreting results."""
    if limit < 1:
        raise ValueError("limit must be >= 1")
...
def list_formal_property_results(
    ...
    *,
    ...
    """Return property-level evidence for one persisted formal snapshot."""
    query = """
