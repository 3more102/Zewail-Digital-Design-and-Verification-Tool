...
def historical_run_snapshot(
    ...
    """Return the persisted run evidence bound into a reviewed rerun action."""
...
def _validated_snapshot_inputs(
    ...
    if not isinstance(identity, dict) or not isinstance(inputs, dict):
        raise ValueError("Historical run snapshot is missing identity or recorded inputs.")

    identity_source = {
        "project": identity.get("project"),
        "simulator": identity.get("simulator"),
        "top": identity.get("top"),
    }
    identity_errors = _historical_identity_errors(project, identity_source)
    if identity_errors:
        raise ValueError(
            "Historical run identity mismatch: " + "; ".join(identity_errors)
        )
...
def rerun_records(
    ...
    *,
    ...
    """Build once, then rerun the selected records' stored runtime inputs."""
...
def rerun_snapshot(
    ...
    *,
    ...
    """Rerun exactly the runtime inputs bound into a reviewed historical snapshot."""
...
def rerun_run_id(
    ...
    *,
    ...
    """Convenience API: snapshot one compatible run, then rerun its stored inputs."""
