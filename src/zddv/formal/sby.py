...
def render_sby_bmc_config(
    ...
    """Render one explicit finite-depth SymbiYosys BMC job."""

    if request.mode != "bmc":
        raise ValueError(
            "SymbiYosys bounded execution currently supports bmc mode only"
        )
    if request.depth is None:
        raise ValueError(
            "SymbiYosys bounded checks require an explicit depth"
        )
    ...
    for source, destination in hdl:
        if source.suffix.lower() == ".sv":
            script_lines.append(
                f"read_verilog -formal -sv {destination}"
            )
        else:
...
class SymbiYosysBackend(FormalBackend):
    """Finite-depth SymbiYosys BMC backend."""
    ...
    def version(self) -> str:
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                "Unable to query SymbiYosys version "
    ...
    def check(
        ...
        if outcome.timed_out:
            if output and not output.endswith("\n"):
                output += "\n"
            output += (
                "ZDDV: formal check timed out before "
                "SymbiYosys completed\n"
            )
