...
def render_sby_bmc_config(
    ...
    """Render one finite-depth SBY BMC job without strengthening its claims."""
    if request.mode != "bmc":
...
def _collect_artifacts(config_path: Path, run_dir: Path) -> tuple[Path, ...]:
    candidates: list[Path] = [config_path]
    for name in ("status", "logfile.txt", "config.sby"):
        path = run_dir / name
        if path.is_file():
            candidates.append(path)
    for pattern in ("**/trace.vcd", "**/trace.fst", "**/trace.yw", "**/trace.smtc"):
...
class SymbiYosysBackend(FormalBackend):
    """Concrete ZDDV formal backend for finite-depth SymbiYosys BMC."""
    ...
    def check(
        ...
        return FormalCheckResult(
            ...
            log_path=log_path,
            properties=(),
            artifacts=_collect_artifacts(config_path, run_dir),
