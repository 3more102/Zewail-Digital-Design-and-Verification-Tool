...
def _manifest_path(project: ProjectConfig, value: str | Path) -> Path:
    ...
    try:
        ...
        raise ValueError(
            "Desktop generated-review manifests must remain under .zddv/generated/drafts"
        ) from exc
    if path.name != "manifest.json":
        raise ValueError("Desktop generated-review path must identify a generated manifest.json")
    return path
...
def inspect_desktop_generated_draft(
...
def list_desktop_generated_drafts(
    ...
    *,
    ...
    for manifest_file in root.glob("*/manifest.json"):
        try:
            row = inspect_desktop_generated_draft(project, manifest_file)
...
def apply_desktop_generated_draft(
    ...
    *,
    ...
    draft = inspect_desktop_generated_draft(project, manifest_path)
...
def attach_desktop_generated_review_tab(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "Desktop generated-review actions require Python with tkinter support installed."
        ) from exc
    ...
    def stage_proposal() -> None:
        ...
        if not proposal:
            return
        try:
            result = stage_desktop_generated_proposal(project, proposal)
        except (OSError, RuntimeError, ValueError) as exc:
