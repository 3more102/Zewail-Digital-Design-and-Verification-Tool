...
_URG_LINE_MODULE_HEADER = re.compile(
    r"^\s*(?:-+\s*)?Line Coverage for Module\s*:\s*(?P<module>.+?)\s*$",
    re.IGNORECASE,
)
_URG_COVERAGE_SECTION_HEADER = re.compile(
    r"^\s*(?:-+\s*)?(?:Line|Cond|Condition|Toggle|FSM|Branch|Assertion) "
...
def parse_vcs_urg_line_coverage(path: str | Path) -> list[dict]:
    ...
    while index < len(lines):
        ...
        if in_module_line_section and _URG_COVERAGE_SECTION_HEADER.match(raw_line):
            module = None
            in_module_line_section = False
            index += 1
            continue
