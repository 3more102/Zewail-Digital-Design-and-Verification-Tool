...
def _imc_counts(value: str | None) -> dict | None:
    if value is None:
        return None
    ...
    if covered > total:
        raise ValueError(f"IMC covered count exceeds total: {value}")
    return {
        "covered": covered,
        "total": total,
        "hit_rate": 100.0 * covered / total if total else None,
...
def parse_xcelium_imc_summary(text: str) -> dict:
    """Normalize documented cumulative IMC summary grades and explicit counts.

    Average and Covered are retained as distinct simulator-reported grades.
    Overall Covered is the percentage-native history score. Parenthesized
    two-field counts are stored only when IMC reports them explicitly.
    """
    ...
    for prefix in ("overall", "code", "fsm", "functional"):
        counts = _imc_counts(match.group(f"{prefix}_counts"))
        if counts is not None:
            by_metric_counts[f"{prefix}_covered"] = counts
...
def merge_xcelium_coverage(project: ProjectConfig) -> dict:
    """Merge Xcelium coverage with IMC and normalize documented summary evidence."""
