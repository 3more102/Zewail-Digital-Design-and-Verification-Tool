...
def parse_questa_code_coverage_report(text: str) -> list[dict]:
    ...
    normalized conservatively. Documented multibit-verbose expression rows are
    collapsed into input-term-bit points, covered only when both logical FEC
    states have non-zero hits. FSM state/transition rows follow the documented
    v2024.2 text-report sections. Multibit condition rows remain unnormalized.
    """
    points: list[dict] = []
    kind = ""
    source_file = ""
...
def merge_questa_coverage(project: ProjectConfig) -> dict:
    ...
    if (
        multibit_expression_evidence["status"] == "captured"
    ...
    elif multibit_expression_evidence["status"] == "failed":
        multibit_expression_status = "tool-error"
    else:
        multibit_expression_status = str(
            multibit_expression_evidence["status"]
        )
    ...
    payload = {
        ...
        **metrics,
        ...
        "multibit_expression_returncode": int(
            multibit_expression_evidence["returncode"]
        ),
