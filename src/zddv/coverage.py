...
def parse_questa_code_coverage_report(text: str) -> list[dict]:
    ...
    Documented multibit expression reports emitted with
    -code e -details -multibitverbose are normalized per operand bit: a bit
    term is covered only when both its _0 and _1 FEC targets are observed.
    """
    points: list[dict] = []
    kind = ""
    source_file = ""
