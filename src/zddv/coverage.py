...
def parse_xcelium_imc_summary(text: str) -> dict:
    """Normalize only explicitly labelled top-level IMC Overall scores."""
...
def merge_xcelium_coverage(project: ProjectConfig) -> dict:
    """Merge Xcelium run databases and normalize explicit IMC Overall scores.

    The native merge/report artifacts remain the source of truth. Numeric
    history is recorded only when the summary exposes the documented
    Overall Average/Covered header and a parseable percentage row.
    """
