...
_QUESTA_FSM_UNCOVERED_STATE_ROW = re.compile(
    r"^\s*(?P<state>\S+)\s*$"
)
...
def parse_questa_fsm_coverage_report(text: str) -> list[dict]:
    ...
    are accepted. Summary percentages and unlabeled numeric rows are retained
    only in the raw simulator report and are not reinterpreted here.
    """
    points: list[dict] = []
    scope = ""
    fsm_id = ""
    ...
    for raw_line in text.splitlines():
        ...
        if section in {"covered-transitions", "uncovered-transitions"}:
            ...
            points.append(
                {
                    "name": (
                        f"{scope}|{identity()}|transition:{trans_id} "
                        f"{transition}"
                    ),
                    "count": hits,
                    "hit": hits > 0,
...
def merge_questa_coverage(project: ProjectConfig) -> dict:
    ...
    fsm_report_path.write_text(
        fsm_report.stdout or "",
        encoding="utf-8",
    )
    fsm_points = (
