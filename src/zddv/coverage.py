        ...
        "holes": [
            {
                ...
                **(
                    ...
                    if point.get("scope") is not None
                    else {}
                ),
                **(
...
def parse_questa_statement_coverage_xml(path: str | Path) -> list[dict]:
    """Normalize documented by-instance statement details from vcover XML."""
    ...
    for element in root.iter():
        ...
        if file_number is not None:
            key = str(file_number)
            if instance is not None:
                source_file = instance_files.get(instance, {}).get(key)
            if source_file is None:
                source_file = global_files.get(key)

        location = source_file or (
            f"file#{file_number}" if file_number is not None else "<unknown-source>"
        )
        name = f"{location}:{line}:{item}"
        if scope:
            name = f"{scope}|{name}"
...
def write_questa_statement_hole_report(
    ...
    *,
    ...
    """Generate by-instance statement details and write normalized zero-hit holes."""
