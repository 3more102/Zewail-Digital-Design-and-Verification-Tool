...
def write_questa_statement_hole_report(
    ...
    *,
    ...
    """Write zero-hit statement holes from the retained detailed Questa XML."""
    ...
    if not details_path.exists():
        raise RuntimeError(
            ...
            "Run 'zddv coverage' first and verify that detailed XML capture is available."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Detailed Questa coverage XML is malformed: {details_path}"
        ) from exc

    if not points:
        raise RuntimeError(
            f"No statement coverage items could be normalized from {details_path}."
        )
