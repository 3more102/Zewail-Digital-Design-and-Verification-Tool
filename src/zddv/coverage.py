...
def parse_questa_code_coverage_report(text: str) -> list[dict]:
    ...
    Rows/Hits/FEC Target table header. Multibit expression rows follow
    Questa's documented multibit-verbose layout: one FEC-target row per
    logical state and one hit-count column per bit index. ZDDV collapses each
    _0/_1 pair into the vendor's input-term-bit coverage unit, which is
    covered only when both state rows have a non-zero hit count.
    """
    points: list[dict] = []
    kind = ""
    source_file = ""
...
def merge_questa_coverage(project: ProjectConfig) -> dict:
    ...
    multibit_evidence = detailed_code_coverage_evidence["multibit_expression"]
    multibit_expression_points: list[dict] = []
    if multibit_evidence["status"] == "captured":
        ...
        multibit_expression_status = (
            "ok" if multibit_expression_points else "empty"
        )
    elif multibit_evidence["status"] == "failed":
        multibit_expression_status = "tool-error"
    else:
        multibit_expression_status = "missing"
