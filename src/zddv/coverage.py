...
def parse_questa_code_coverage_report(text: str) -> list[dict]:
    ...
    Documented multibit-expression verbose rows are normalized per input-term
    bit while preserving both FEC target hit counts and non-masking evidence.
    """
    points: list[dict] = []
    kind = ""
    source_file = ""
    ...
    def flush_multibit_points() -> None:
        ...
        for multibit_row in multibit_rows:
            target = str(multibit_row["target"])
            match = re.match(
    ...
    for raw_line in text.splitlines():
        ...
        if (
            normalized_line.startswith("rows: fec target")
            and kind == "expression"
        ):
            in_fec_rows = False
...
def merge_questa_coverage(project: ProjectConfig) -> dict:
    ...
    if multibit_expression_evidence["status"] == "captured":
    ...
    multibit_expression_status = (
        ...
        if multibit_expression_evidence["status"] == "captured"
        else str(multibit_expression_evidence["status"])
    )
