...
def parse_questa_statement_coverage_xml(path: str | Path) -> list[dict]:
    """Normalize statement items from a detailed Questa vcover XML report."""
    ...
    for element in root.iter():
        if _xml_local_name(element.tag) not in {"file", "fileMap"}:
            continue
        number = element.attrib.get("fn")
        file_path = element.attrib.get("path")
        if number is None or not file_path:
            continue
        instance = _xml_ancestor(element, parents, {"instance", "instanceData"})
        if instance is None:
            global_files[str(number)] = str(file_path)
        else:
            instance_files.setdefault(instance, {})[str(number)] = str(file_path)
    ...
    for element in root.iter():
        ...
        instance = _xml_ancestor(element, parents, {"instance", "instanceData"})
        scope = str(instance.attrib.get("path") or "").strip() if instance is not None else ""
        number = element.attrib.get("fn")
        source_path = None
        ...
        location = source_path or (f"file#{number}" if number is not None else "<unknown-source>")
        if line is not None:
            location += f":{line}"
...
def write_questa_statement_hole_report(
    ...
    *,
    ...
    """Write zero-hit statement holes from retained detailed Questa XML evidence."""
    out_dir = (project.root / ".zddv" / "coverage").resolve()
    merged_path = out_dir / "coverage.ucdb"
    details_path = out_dir / "details.xml"
    zeros_path = out_dir / "zeros.txt"

    if not merged_path.exists():
        raise RuntimeError(
            f"Merged Questa coverage not found at {merged_path}. Run 'zddv coverage' first."
        )
    if not details_path.exists():
        raise RuntimeError(
            f"Detailed Questa coverage XML not found at {details_path}. "
            "Run 'zddv coverage' first and verify detailed XML capture."
        )

    try:
        points = parse_questa_statement_coverage_xml(details_path)
    except ET.ParseError as exc:
        raise RuntimeError(f"Detailed Questa coverage XML is malformed: {details_path}") from exc
    if not points:
        raise RuntimeError(f"No statement coverage items could be normalized from {details_path}.")

    report = build_coverage_hole_report(points, point_type="statement", limit=limit)
    destination = Path(output)
    ...
    payload = {
        **report,
        ...
        "xml": str(details_path),
        "zeros": str(zeros_path) if zeros_path.exists() else None,
    }
