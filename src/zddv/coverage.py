...
def parse_questa_statement_coverage_xml(path: str | Path) -> list[dict]:
    """Normalize statement items from a by-instance Questa vcover XML report."""
    source = Path(path)
    root = ET.parse(source).getroot()
    points: list[dict] = []

    for instance_index, instance in enumerate(root.iter()):
        ...
        for statement_index, element in enumerate(instance.iter()):
            ...
            if raw_statement is not None:
                try:
                    statement_number = int(raw_statement)
                except ValueError:
                    pass
            ...
            points.append(
                {
                    "type": "statement",
                    "name": (
                        f"{scope}|{location}:{line_label}:stmt{statement_label}"
                    ),
                    "count": hits,
                    "hit": hits > 0,
                    "scope": scope,
                    "file": source_path,
                    "file_id": (
                        str(file_number) if file_number is not None else None
                    ),
                    "line": line_number,
    ...
    points.sort(
        key=lambda point: (
            ...
            -1
            if point.get("statement") is None
            else int(point["statement"]),
...
def write_questa_statement_hole_report(
    ...
    *,
    ...
    """Export by-instance statement XML and report zero-hit statement items."""
    ...
    evidence_dir = (project.root / ".zddv" / "coverage" / "questa").resolve()
    evidence_dir.mkdir(parents=True, exist_ok=True)
    xml_path = evidence_dir / "statement-details.xml"
    if xml_path.exists():
        xml_path.unlink()
    ...
    for hole in report["holes"]:
        source_point = holes_by_name.get(str(hole["name"]), {})
        enriched_holes.append(
            {
                **hole,
                "scope": source_point.get("scope"),
                "file": source_point.get("file"),
                "line": source_point.get("line"),
                "statement": source_point.get("statement"),
            }
        )
