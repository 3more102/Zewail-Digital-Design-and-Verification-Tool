...
def parse_xcelium_imc_summary(text: str) -> dict:
    """Normalize cumulative grades and explicit counts from an IMC summary.

    IMC reports Average and Covered as distinct grades. ZDDV retains both and
    uses Overall Covered as the percentage-native history score. Parenthesized
    covered/total counts are persisted only when IMC reports them explicitly.
    """
    ...
    if header_index is None:
        raise ValueError(
            "IMC summary header with Overall/Code/FSM/Functional "
            "Average/Covered grades was not found"
        )
    ...
    return {
        ...
        "metric_semantics": "imc-summary-average-covered",
    }


def merge_xcelium_coverage(project: ProjectConfig) -> dict:
    """Merge captured Xcelium run databases and normalize IMC summary metrics."""
