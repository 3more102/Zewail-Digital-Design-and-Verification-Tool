...
def parse_questa_code_coverage_report(text: str) -> list[dict]:
    """Normalize statement, branch, condition, and scalar expression items.

    Condition/expression coverage uses focused-expression-coverage (FEC)
    rows. Only rows after the explicit FEC table header are normalized so
    expression truth-table rows are retained as evidence but not mistaken for
    coverage goals.
    """
    points: list[dict] = []
    kind = ""
