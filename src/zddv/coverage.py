...
def parse_questa_code_coverage_report(text: str) -> list[dict]:
    """Normalize documented Questa statement/branch detail rows.

    The parser intentionally handles only statement and branch sections whose
    text shape is documented in the Questa SIM User's Manual. Context rows such
    as "Count coming in to IF" are skipped because they are not independently
    coverable branch items.
    """
    points: list[dict] = []
    kind = ""
    source_file = ""
