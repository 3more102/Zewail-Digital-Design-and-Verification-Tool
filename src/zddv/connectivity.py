...
def qualify_signal_navigation_with_elaboration(
    ...
    *,
    ...
    instance-port edge is correlated only when parent path, exact child path,
    child module, port, and parent signal all agree.
    """
    ...
    def direct_pin_candidates(
        ...
        *,
        ...
        port = entry.get("port")
        child_type = entry.get("child_type")
        parent_signal = entry.get("signal")
        ...
        for binding in normalized_pin_bindings:
            ...
            if str(binding.get("instance_path") or "") != child_path:
                continue
            if child_type and str(binding.get("instance_module") or "") != str(child_type):
                continue
