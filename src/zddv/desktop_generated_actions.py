...
def list_reviewable_generated_drafts(project: ProjectConfig) -> list[dict[str, Any]]:
    """List staged generated-verification drafts without changing project state."""
...
def apply_reviewed_generated_draft(
    ...
    *,
    ...
    """Apply only through the existing SHA-confirmed core review gate."""
...
def attach_desktop_generated_actions_tab(notebook: Any, project: ProjectConfig) -> None:
    """Attach review-gated generated-verification draft apply controls."""
    try:
    ...
    intro = (
        "No automatic project changes. Applying generated verification code requires "
        "a staged DRAFT, intact content, explicit review approval, and the exact SHA-256."
    )
    ...
    for column, title, width in (
        ...
        ("kind", "Kind", 100),
        ("name", "Draft", 220),
        ("target", "Suggested target", 320),
        ("sha", "Content SHA-256", 420),
    ):
    ...
    ttk.Checkbutton(
        form,
        text="I reviewed the exact staged content and confirm this SHA-256",
    ...
    def refresh_drafts() -> None:
        ...
        status_var.set(
            ...
            if not rows
            else f"{len(rows)} staged draft(s) found. Selection does not modify the project."
        )
