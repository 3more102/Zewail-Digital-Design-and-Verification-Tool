...
from zddv.storage import list_runs, run_statistics


def _failure_groups(rows: list[dict]) -> list[tuple[str, int]]:
...
def generate_html_report(project: ProjectConfig, *, limit: int = 100) -> dict:
    rows = list_runs(project, limit=limit)
...
"""
    ...
    return {
        ...
        "stats": stats,
        "failure_groups": [{"signature": sig, "count": count} for sig, count in failures],
        "shown_runs": len(rows),
    }
