...
> Status: **v0.6 Multi-Simulator UVM + Coverage Foundation — run-aware UVM phase/objection and sequence-item history, normalized sequence lifecycles, observed grant-order plus explicit contention-window reconstruction, Questa UCDB/code/functional coverage normalization, and Verilator/Questa/VCS/Xcelium execution with native VCS coverage plus normalized URG score/count history**
...
- Simulator-independent UVM sequence-item handshake normalization with run correlation, SQLite snapshot/event/violation persistence, history filtering, and observed GRANT-order evidence
