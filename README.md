...
> Status: **v0.5 Protocol Verification — APB, AXI4-Lite, burst-aware AXI4 with exclusive-access, request-sideband, and WSTRB byte-lane checks, async-FIFO CDC invariants, and public UCIe FLIT/link-health analysis**
...
- AXI4 burst analysis with IDs, lengths/types, WLAST/RLAST, 4KB-boundary checks, core exclusive-access semantics, AxCACHE/AxPROT/AxQOS/AxREGION validation, and WSTRB byte-lane legality when write-bus width is known
...
- [x] AXI4 WSTRB write-bus width and per-beat byte-lane legality checks
...
When a normalized trace supplies `write_data_bus_bytes`, the analyzer also checks
that AWSIZE does not exceed the write-data bus and that each accepted WSTRB value
fits the bus width and enables only byte lanes belonging to that write beat.
Partial writes, including WSTRB=0, remain legal. `axi4-waveform` derives
`write_data_bus_bytes` automatically from the declared VCD WSTRB width.
