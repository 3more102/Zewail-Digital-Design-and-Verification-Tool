...
> Status: **v0.6 UVM + Questa Adapter Foundation — run-aware UVM ingestion with phase/objection lifecycle traces plus conservative sequence report-context evidence, native Questa build/run orchestration, UCDB capture, and summary-level UCDB merge/report normalization on top of the v0.5 protocol-verification foundation**
...
- UVM phase/objection lifecycle normalization from standard `+UVM_PHASE_TRACE` / `+UVM_OBJECTION_TRACE` report evidence, plus explicit `sequencer@@sequence` report-context evidence, persisted in SQLite
...
- [x] Conservative sequence-aware UVM report-evidence model when explicit `sequencer@@sequence` context is present
