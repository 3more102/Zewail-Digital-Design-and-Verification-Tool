...
> Status: **v0.6 UVM + Questa Adapter Foundation — run-aware UVM ingestion with phase/objection lifecycle traces, native Questa build/run orchestration, UCDB capture/merge, functional-bin ingestion, detailed XML evidence, and statement-level coverage-hole reporting on top of the v0.5 protocol-verification foundation**
...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, retained detailed XML evidence, and statement-level source coverage-hole normalization
...
a separate simulator-reported value. The retained `details.xml` is also used by
`zddv coverage-holes --type statement` to report zero-hit statements with instance
scope, source file, line, statement index, and hit count. Branch, condition,
expression, toggle, and FSM item-level hole normalization remain planned.
...
# Questa: statement holes from retained detailed coverage XML
