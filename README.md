...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, retained detailed evidence, and documented branch-item normalization for code `coverage-holes`; other code item classes remain pending
...
report for zero-hit source/file-line evidence. In addition, ZDDV captures the documented
branch-detail text report and normalizes branch line/item/hit rows into the shared
`coverage-holes` flow. Statement, condition, expression, toggle, and FSM item-level
normalization remain pending. Questa's weighted total coverage remains a separate
...
- [x] Questa branch item/source normalization and coverage-hole reporting
- [ ] Questa statement/condition/expression/toggle/FSM detailed item normalization
- [ ] VCS adapter
- [ ] Xcelium adapter
- [ ] Formal adapter API
