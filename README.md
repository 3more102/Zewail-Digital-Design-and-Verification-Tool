...
- Simulator-independent UVM sequence-item handshake normalization with run correlation, SQLite snapshot/event persistence, history filtering, violation queries, and observed GRANT-order evidence without policy inference
- Evidence-first UVM arbitration/fairness analysis from explicit contender/grant decisions, with project-defined wait bounds, per-request metrics, run correlation, and SQLite history
