...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, ordinary covergroup-bin ingestion, retained machine-readable XML, and complementary zero-hit source/file-line evidence; detailed code-item/source hole normalization is still pending
...
inspect those normalized bins. ZDDV also retains the documented XML coverage
report plus a complementary `-zeros -details -codeAll` report for zero-hit
source/file-line evidence. Questa's weighted total coverage is preserved as a
separate simulator-reported value; detailed code-item/source `coverage-holes`
remains pending.
...
- [x] Questa detailed XML coverage evidence retention
- [x] Questa zero-hit source/file-line evidence retention
