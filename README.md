...
inspect those normalized bins. ZDDV also retains two complementary detailed code-coverage
evidence artifacts: XML detail output for machine-readable follow-up and a `-zeros -details`
text report for zero-hit source/file-line evidence. These artifacts are retained but not yet
normalized into `coverage-holes`. Questa's weighted total coverage remains a separate
...
- [x] Questa detailed code-coverage evidence retention (XML + zero-hit source detail report)
