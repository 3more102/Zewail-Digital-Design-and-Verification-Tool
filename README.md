...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, retained XML/zero-hit evidence, and statement-level source coverage-hole normalization
...
report for zero-hit source/file-line evidence. `zddv coverage-holes --type
statement` normalizes statement records from the retained XML into zero-hit
instance/source/line reports while preserving the complementary zero-detail artifact.
...
# Questa statement holes from retained detailed XML
