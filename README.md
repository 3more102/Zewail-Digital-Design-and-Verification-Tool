...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, ordinary covergroup-bin ingestion, retained detailed XML/zero-hit evidence, and statement/branch zero-hit normalization for shared `coverage-holes` reporting
...
and branch zero-hit rows from that report into `coverage-holes`; other detailed code
coverage classes remain evidence-only for now. Questa's weighted total coverage
...
- [x] Questa statement/branch zero-hit item/source normalization and coverage-hole reporting
