...
- Synopsys VCS native `vcs` -> `simv` build/run foundation with version detection, UVM 1.2 compilation, deterministic seeds, plusargs, timeouts, VCD capture, assertion ingestion, run-linked UVM normalization, per-run native `.vdb` coverage capture, multi-run URG merge/report evidence, normalized dashboard scores, documented global covergroup type/instance counts, documented module-level line/condition/toggle/FSM-transition/branch covered/total counts from `modinfo.txt`, deduplicated instance-level line/condition/toggle/branch plus separate FSM state/transition/sequence counts from URG HTML detail, and explicit condition truth-table coverage-hole normalization
...
# VCS: explicit URG condition truth-table holes
zddv --project my_project coverage-holes --type condition --output .zddv/coverage/condition-holes.json
...
- [x] VCS documented module-level line/condition/toggle/FSM-transition/branch covered/total count ingestion from `modinfo.txt`
- [x] VCS instance-level line/condition/toggle/branch plus separate FSM state/transition/sequence count aggregation from URG HTML detail
- [x] VCS condition truth-table item normalization and `coverage-holes --type condition`
- [ ] VCS line/branch/toggle/FSM item-level hole normalization
