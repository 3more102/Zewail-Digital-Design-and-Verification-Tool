...
- Synopsys VCS native `vcs` -> `simv` build/run foundation with version detection, UVM 1.2 compilation, deterministic seeds, plusargs, timeouts, VCD capture, assertion ingestion, run-linked UVM normalization, per-run native `.vdb` coverage capture, multi-run URG merge/report evidence, normalized dashboard scores, and documented global covergroup type/instance counts, plus documented LINE/COND/TOGGLE/FSM/BRANCH covered/coverable counts from a separate URG ratio report
...
- [x] VCS code-metric covered/total object-count ingestion via documented URG `-show ratios` report evidence
- [ ] Xcelium adapter
- [ ] Formal adapter API
