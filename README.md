...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, and retained detailed XML coverage evidence; code item/source hole normalization is still pending
...
`vcover report -xml -output` flow and retains the result at
`.zddv/coverage/details.xml` as raw machine-readable evidence. Questa's
weighted total coverage is preserved as a separate simulator-reported value;
code item/source `coverage-holes` remains pending until that evidence is
normalized.
