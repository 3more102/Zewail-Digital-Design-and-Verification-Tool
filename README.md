...
and dedicated by-instance toggle detail/XML. ZDDV normalizes documented
statement and branch rows, scalar condition/expression FEC rows, multibit
expression FEC rows, FSM state/transition rows, and the documented 1H->0L and
0L->1H toggle-report transitions into `zddv coverage-holes`. Multibit
condition FEC normalization remains pending. Questa's weighted total coverage
...
- [x] Questa statement/branch, scalar condition/expression FEC, multibit-expression FEC, FSM, and toggle-transition item normalization
- [ ] Questa multibit condition FEC normalization
