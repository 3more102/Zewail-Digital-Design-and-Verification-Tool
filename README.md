...
- Synopsys VCS native `vcs` -> `simv` build/run foundation with version detection, UVM 1.2 compilation, deterministic seeds, plusargs, timeouts, VCD capture, assertion ingestion, run-linked UVM normalization, per-run native `.vdb` coverage capture, multi-run URG merge/report evidence, normalized dashboard scores, documented global covergroup type/instance counts, documented module-level line/branch/condition/toggle covered/total counts from `modinfo.txt`, and instance-level code-metric counts from URG HTML detail reports
...
- [x] VCS documented module-level line/branch/condition/toggle covered/total count ingestion from `modinfo.txt`
- [x] VCS instance-level LINE/COND/TOGGLE/FSM/BRANCH object-count ingestion from URG HTML detail reports
- [ ] VCS FSM module-level covered/total count normalization
