...
> Status: **v0.6 UVM + Questa Adapter Foundation — run-aware UVM ingestion with phase/objection lifecycle traces, native Questa build/run orchestration, UCDB capture, summary/functional coverage normalization, and retained detailed XML evidence on top of the v0.5 protocol-verification foundation**
...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, and retained `vcover report -xml` evidence; detailed code-item/source hole normalization is still pending
...
functional-coverage database. It also attempts the documented
`vcover report -xml -output ...` export and retains the XML as raw
machine-readable evidence without guessing undocumented UCDB internals.
`zddv fcov-history` and `zddv fcov-holes` can inspect normalized functional
bins. Questa's weighted total coverage is preserved as a separate
simulator-reported value; detailed code-item/source `coverage-holes` normalization
from the retained XML remains pending.
