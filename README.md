...
ZDDV can also decode APB directly from a VCD waveform. `apb-waveform`
samples signals on PCLK edges, emits the same normalized trace model, then runs
the same protocol checker. If exactly one waveform scope contains PCLK, PSEL,
and PENABLE, the scope is selected automatically; otherwise use `--scope`.
