...
- Synopsys VCS native `vcs` -> `simv` build/run foundation with version detection, UVM 1.2 compilation, deterministic seeds, plusargs, timeouts, VCD capture, assertion ingestion, run-linked UVM normalization, per-run native `.vdb` coverage capture, multi-run URG merge/report evidence, normalized dashboard scores, documented global covergroup type/instance counts, and documented module/instance line/condition/toggle/FSM/branch covered/total counts from `modinfo.txt`
...
- [x] VCS condition/toggle/FSM module counts and instance-level line/condition/toggle/FSM/branch aggregation from `modinfo.txt`
- [ ] VCS source-linked code-metric hole normalization from detailed URG reports
- [ ] Xcelium adapter
- [ ] Formal adapter API
