...
report for zero-hit source/file-line evidence. ZDDV also normalizes statement/branch rows plus scalar condition/expression
focused-expression-coverage (FEC) rows from
`vcover report -details -dumptables -code sbce`, allowing
`zddv coverage-holes` to report source-linked misses for all four metrics.
Multibit condition/expression FEC layouts plus toggle/FSM item normalization
remain pending. Questa's weighted total coverage remains a separate
