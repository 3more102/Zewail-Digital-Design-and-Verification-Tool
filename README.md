...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, retained machine-readable detailed XML evidence, and complementary zero-hit source/file-line detail evidence; detailed code-item/source hole normalization is still pending
...
inspect those normalized bins. ZDDV also retains the detailed XML export and a
complementary `vcover report -zeros -details -codeAll` report for zero-hit
source/file-line evidence. These code-coverage artifacts are retained as evidence
only; detailed code-item/source `coverage-holes` remains pending. Questa's weighted
total coverage stays separate as the simulator-reported score.
