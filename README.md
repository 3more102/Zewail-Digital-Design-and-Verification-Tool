...
- Formal adapter API with normalized check/property/result contracts, persisted evidence history, finite-depth SymbiYosys BMC execution, direct cover-property reachability, native VCD counterexample/witness normalization, and automatic run-local normalization of backend-reported VCD traces during result persistence
...
- [x] Counterexample/witness normalization from normalized JSON and native VCD traces, including automatic backend-reported VCD normalization during persistence
