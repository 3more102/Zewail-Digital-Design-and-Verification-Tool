...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, ordinary covergroup-bin ingestion, retained detailed evidence, and normalized statement/branch code-coverage holes; condition/expression/toggle/FSM item normalization remains pending
...
report for zero-hit source/file-line evidence. ZDDV also parses documented detailed
statement and branch report rows into normalized code points, so `zddv coverage-holes`
can report zero-hit statements and branches with source file, line, item index, and
detail text. Condition/expression/toggle/FSM item-level normalization remains pending.
...
- [x] VCS adapter foundation and native per-run coverage database capture
