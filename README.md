...
- Synopsys VCS native `vcs` -> `simv` build/run foundation with version detection, UVM 1.2 compilation, deterministic seeds, plusargs, timeouts, VCD capture, assertion ingestion, run-linked UVM normalization, per-run native `.vdb` coverage capture, multi-run URG merge/report evidence, normalized dashboard scores, documented global covergroup type/instance counts, module-level line/branch/condition/toggle-bit counts from `modinfo.txt`, deduplicated instance-level line/condition/toggle/branch plus separate FSM state/transition/sequence counts from URG HTML detail, and retained URG brief uncovered-object evidence
...
- [x] VCS instance-level line/condition/toggle/branch counts plus separate FSM state/transition/sequence counts from URG HTML detail
- [x] VCS documented module-level condition/toggle-bit count ingestion from `modinfo.txt`
- [x] VCS retained URG brief uncovered-object evidence
- [ ] VCS module-definition FSM normalization
