...
statement/branch and scalar condition/expression FEC rows from `vcover report -details -code sbce`, allowing
`zddv coverage-holes` to report source-linked statement/branch misses and scalar condition/expression FEC-row misses.
Multibit condition/expression rows and toggle/FSM item normalization remain pending. Questa's
