...
- Synopsys VCS native `vcs` -> `simv` build/run foundation with version detection, UVM 1.2 compilation, deterministic seeds, plusargs, timeouts, VCD capture, assertion ingestion, run-linked UVM normalization, per-run native `.vdb` coverage capture, multi-run URG merge/report evidence, normalized dashboard scores, documented global covergroup type/instance counts, and documented module-level line/branch/condition/toggle-bit covered/total counts from `modinfo.txt`
...
- [x] VCS documented module-level condition and toggle-bit covered/total count ingestion from `modinfo.txt`
- [ ] VCS remaining FSM module counts and instance-level code-metric aggregation
- [ ] Xcelium adapter
- [ ] Formal adapter API
