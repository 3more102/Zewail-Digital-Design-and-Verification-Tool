...
- Questa per-run UCDB capture, multi-run `vcover merge`, normalized `vcover report -summary` metrics, automatic ordinary covergroup-bin ingestion from `vcover report -cvg -details`, retained machine-readable detailed XML evidence, and statement-level source hole normalization; other code-item types remain pending
...
a separate simulator-reported value. `zddv coverage-holes --type statement`
can export statement-only XML evidence from the merged UCDB and report zero-hit
statements with instance scope, source file, line, statement index, and hit count.
Branch/condition/expression/toggle/FSM item-level hole normalization remains planned.
...
- [x] Questa statement-level XML code-coverage normalization and coverage-hole reporting
- [ ] Questa branch/condition/expression/toggle/FSM item-level normalization
- [ ] VCS adapter
- [ ] Xcelium adapter
- [ ] Formal adapter API
