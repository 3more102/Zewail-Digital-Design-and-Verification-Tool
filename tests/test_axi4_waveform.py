...
def test_extracts_burst_aware_axi4_trace_from_vcd():
    ...
    assert trace["address_width_bits"] == 12
    assert trace["waveform"]["address_width_bits"] == 12
...
def test_axi4_waveform_keeps_partial_id_pair_width_unknown(tmp_path: Path):
    source = FIXTURE.read_text(encoding="utf-8")
...
def test_axi4_waveform_does_not_infer_zero_id_width_from_omitted_declarations(
    tmp_path: Path,
):
    source = FIXTURE.read_text(encoding="utf-8")
