...
def test_axi4_waveform_keeps_partial_id_signal_pair_unknown(tmp_path: Path):
    source = FIXTURE.read_text(encoding="utf-8")
...
def test_axi4_waveform_does_not_infer_zero_from_undumped_id_pair(tmp_path: Path):
    source = FIXTURE.read_text(encoding="utf-8")
    source = source.replace("$var wire 2 d AWID [1:0] $end\n", "")
    source = source.replace("$var wire 2 p BID [1:0] $end\n", "")
    undumped = tmp_path / "undumped_write_id_pair.vcd"
    undumped.write_text(source, encoding="utf-8")

    trace = extract_axi4_trace_from_vcd(undumped)

    assert "ID_W_WIDTH" not in trace["id_widths"]
    assert trace["waveform"]["id_widths"] == {"ID_R_WIDTH": 2}
