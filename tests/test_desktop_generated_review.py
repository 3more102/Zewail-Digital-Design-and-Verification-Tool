...
from zddv.desktop_generated_review import (
    ...
    inspect_desktop_generated_draft,
...
def test_desktop_generated_review_stage_list_preview_and_apply_use_core_gates(tmp_path: Path):
    project = initialize_project(tmp_path / "demo")
    staged = stage_desktop_generated_proposal(project, _proposal(project))
    ...
    detail = inspect_desktop_generated_draft(
        project,
    ...
    rows_after = list_desktop_generated_drafts(project)
...
def test_desktop_generated_review_detects_changed_staged_bytes(tmp_path: Path):
    project = initialize_project(tmp_path / "demo")
    staged = stage_desktop_generated_proposal(project, _proposal(project))
    ...
    detail = inspect_desktop_generated_draft(project, staged["manifest_path"])
...
def test_desktop_generated_review_rejects_manifest_outside_generated_drafts(tmp_path: Path):
    project = initialize_project(tmp_path / "demo")
    outside = project.root / "manifest.json"
    outside.write_text("{}\n", encoding="utf-8")

    with pytest.raises(ValueError, match="must remain under"):
        inspect_desktop_generated_draft(project, outside)


def test_desktop_generated_review_limit_is_validated(tmp_path: Path):
    project = initialize_project(tmp_path / "demo")
