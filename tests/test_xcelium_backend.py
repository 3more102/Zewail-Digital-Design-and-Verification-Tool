...
def test_xcelium_build_uses_elaborate_snapshot_flow(tmp_path, monkeypatch):
    ...
    assert command[:4] == ["xrun", "-64bit", "-sv", "-uvm"]
    assert command[command.index("-top") + 1] == "tb_top"
    assert command[command.index("-xmlibdirname") + 1] == str(result.artifact)
    ...
    assert manifest["uvm_library"] == "xcelium-builtin"
    assert manifest["waveform_capture"] == "vcd-tcl"
    assert manifest["coverage_capture"] == "disabled"


def test_xcelium_build_without_waveform_omits_access(tmp_path, monkeypatch):
    ...
    def fake_run(command, **kwargs):
        captured["command"] = list(command)
        Path(command[command.index("-xmlibdirname") + 1]).mkdir(parents=True)
    ...
    assert result.passed is True
    assert "-access" not in captured["command"]


def test_xcelium_run_preserves_seed_plusargs_waveform_and_uvm(tmp_path, monkeypatch):
    project = _project(tmp_path)
    backend = XceliumBackend()
    library_dir = (
        project.root / ".zddv" / "build" / "xcelium.d"
...
"""
    ...
    assert tcl.rstrip().endswith("run")
...
def test_xcelium_requested_coverage_is_not_falsely_claimed(tmp_path, monkeypatch):
    ...
    assert run_record["coverage_capture"] == "pending-native-support"
...
def test_xcelium_version_prefers_xrun_version(monkeypatch):
    backend = XceliumBackend()
    monkeypatch.setattr(backend, "_tool", lambda: "xrun")
    ...
    assert version == "xrun(64): 25.09 test"
    assert commands == [["xrun", "-version"]]


def test_xcelium_version_falls_back_to_explicit_64bit(monkeypatch):
    backend = XceliumBackend()
    monkeypatch.setattr(backend, "_tool", lambda: "xrun")
    commands: list[list[str]] = []
    ...
    def fake_run(command, **kwargs):
        commands.append(list(command))
        if command == ["xrun", "-version"]:
            return SimpleNamespace(
                returncode=1,
                stdout="",
                stderr="launcher requires 64-bit\n",
            )
        return SimpleNamespace(
            returncode=0,
            stdout="xrun(64): 25.09 fallback\n",
            stderr="",
        )
    ...
    assert version == "xrun(64): 25.09 fallback"
    assert commands == [
        ["xrun", "-version"],
        ["xrun", "-64bit", "-version"],
    ]
