...
def test_desktop_crossprobe_rows_render_exact_and_unsupported_pin_evidence():
...
def test_desktop_crossprobe_rows_deduplicate_and_bound_pin_details():
    bindings = [
        {
            "instance_path": f"tb_top.dut{index}",
            "pin": "clk",
            "parent_instance_path": "tb_top",
            "parent_signal": f"clk_{index}",
            "port_direction": "input",
            "relationship": "parent_signal_to_child_input",
        }
    ...
    pin_rows = [detail for kind, detail in rows if kind == "Elaborated pin"]
    assert pin_rows == [
        "tb_top.clk_0 -> tb_top.dut0.clk · input · parent_signal_to_child_input",
        "tb_top.clk_1 -> tb_top.dut1.clk · input · parent_signal_to_child_input",
