...
def test_desktop_elaborated_evidence_rows_use_core_normalized_contract():
...
def test_desktop_elaborated_evidence_rows_preserve_unsupported_expression():
    rows = build_desktop_elaborated_evidence_rows(
        {
            "elaborated_connectivity": {
                "analysis_level": "simulator_elaborated_direct_pin_varref",
    ...
    assert rows == [
        (
            "Elab pin",
            "tb_top.dut.count · UNSUPPORTED · direction=output · "
            "expression=AND · no direct VARREF relation",
        )
    ]


def test_desktop_elaborated_evidence_rows_fail_closed_and_are_bounded():
    assert build_desktop_elaborated_evidence_rows(
        {
            "elaborated_port": {
                "status": "UNAVAILABLE",
    ...
    assert rows[:2] == [
        (
            "Elab pin",
            "tb_top.clk -> tb_top.dut0.clk · input · "
            "parent_signal_to_child_input",
        ),
        (
            "Elab pin",
            "tb_top.clk -> tb_top.dut1.clk · input · "
            "parent_signal_to_child_input",
        ),
    ]
    assert rows[2] == ("Elab pin", "1 additional evidence item(s) not shown")
