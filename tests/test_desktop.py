...
""",
    ...
    record = _run_record("run-wave", "PASS", seed=7)
    record["waveform"] = str(vcd)
    record["run_dir"] = str(project.root / ".zddv" / "runs" / "run-wave")
    record["log"] = str(project.root / ".zddv" / "runs" / "run-wave" / "simulation.log")
...
def test_desktop_waveform_probe_is_bounded_and_validated(tmp_path: Path):
    project = initialize_project(tmp_path / "wave-demo")
    vcd = project.root / "trace.vcd"
    vcd.write_text(
        """$timescale 1ns $end
...
""",
    ...
    record = _run_record("run-wave", "PASS", seed=8)
    record["waveform"] = str(vcd)
    record["run_dir"] = str(project.root / ".zddv" / "runs" / "run-wave")
    record["log"] = str(project.root / ".zddv" / "runs" / "run-wave" / "simulation.log")
    record_run(project, record)
    ...
    assert probe["signals"][0]["changes"] == [{"time": 0, "value": "0"}]
...
def test_desktop_waveform_navigation_reports_missing_evidence(tmp_path: Path):
    project = initialize_project(tmp_path / "wave-demo")
    record_run(project, _run_record("run-no-wave", "PASS", seed=9))
