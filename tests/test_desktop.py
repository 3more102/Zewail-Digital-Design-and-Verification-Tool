...
def test_desktop_marks_elaborated_hierarchy_stale_after_rtl_change(tmp_path: Path):
    project = initialize_project(tmp_path / "elaborated-stale")
    source = project.root / "rtl" / "tb_top.sv"
    source.write_text("module tb_top; endmodule\n", encoding="utf-8")
    project.rtl = ["rtl/*.sv"]
    ...
    source.write_text(
        "module tb_top; logic changed; endmodule\n",
        encoding="utf-8",
    )
