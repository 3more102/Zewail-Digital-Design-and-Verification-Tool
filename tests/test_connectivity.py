...
def test_instance_navigation_correlates_normalized_direct_pin_evidence(tmp_path: Path):
    project = _connectivity_project(tmp_path)
    index = build_connectivity_index(project)
    navigation = signal_navigation(index, unit="top", signal="mid")
    binding = {
    ...
    assert instance_load["elaborated_child_path"] == "top.u_child"
    assert instance_load["elaborated_pin_resolution"] == "exact"
    assert instance_load["elaborated_pin_binding"] == binding
    assert (
        instance_load["elaborated_pin_evidence_contract"]
...
def test_instance_navigation_preserves_ambiguous_direct_pin_matches(tmp_path: Path):
    project = _connectivity_project(tmp_path)
    index = build_connectivity_index(project)
    navigation = signal_navigation(index, unit="top", signal="mid")
    base_binding = {
    ...
    qualified = qualify_signal_navigation_with_elaboration(
        ...
        elaborated_pin_bindings=[
            {**base_binding, "pin_location": {"line": 20}},
            {**base_binding, "pin_location": {"line": 21}},
        ],
        pin_binding_evidence={
            "status": "NORMALIZED",
            "contract": "verilator_cell_pin_direct_varref_only",
    ...
    assert instance_load["role"] == "load"
    assert instance_load["elaborated_pin_resolution"] == "ambiguous"
    assert len(instance_load["elaborated_pin_candidates"]) == 2
