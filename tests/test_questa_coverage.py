...
def test_parse_questa_code_coverage_normalizes_statement_branch_condition_and_expression_items():
    ...
    assert expression_points[0]["fec_target"] == "a_0"
    assert expression_points[3]["hit"] is False
    assert expression_points[3]["fec_target"] == "b_1"
    assert "top.v:30:1:row2 -11" not in {point["name"] for point in points}
...
def test_coverage_holes_cli_supports_questa_statement_branch_condition_and_expression_items(
    tmp_path: Path,
    monkeypatch,
    capsys,
):
    ...
    assert payload["by_type"] == {"branch": 2, "condition": 1, "expression": 1, "statement": 1}
    statement = next(
...
def test_coverage_holes_cli_rejects_unimplemented_questa_item_type(
    ...
    with pytest.raises(
        RuntimeError,
        match=r"supports --type statement, --type branch, --type condition, or --type expression",
    ):
        cmd_coverage_holes(
            SimpleNamespace(
                project=str(project.root),
