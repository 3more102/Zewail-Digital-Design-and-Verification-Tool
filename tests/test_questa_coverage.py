...
def test_coverage_holes_cli_combines_scalar_and_multibit_expression_terms(
    tmp_path: Path,
    monkeypatch,
    capsys,
):
    ...
    assert report["by_type"] == {"expression": 4}
    multibit = [
        hole
        for hole in report["holes"]
        if hole.get("coverage_unit") == "multibit_expression_term"
    ]
    assert len(multibit) == 3
    by_target = {hole["fec_target"]: hole for hole in multibit}
    assert set(by_target) == {"a[1]", "b[0]", "b[2]"}
    assert by_target["b[0]"]["fec_target_hits"] == {"0": 0, "1": 2}
    assert by_target["b[2]"]["fec_rows"] == {"0": 3, "1": 4}
    assert by_target["b[2]"]["multibit_index"] == 2
