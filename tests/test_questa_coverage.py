...
def test_write_questa_statement_hole_report_uses_retained_details_xml(tmp_path: Path):
    project = _project(tmp_path)
    out_dir = project.root / ".zddv" / "coverage"
    out_dir.mkdir(parents=True)
    merged = out_dir / "coverage.ucdb"
    merged.write_text("ucdb fixture\n", encoding="utf-8")
    details = out_dir / "details.xml"
    details.write_text(
        """<?xml version="1.0"?>
<coverage_report>
  <code_coverage_report lines="1" byInstance="1">
...
""",
    ...
    assert report["merged"] == str(merged.resolve())
    assert report["xml"] == str(details.resolve())
    ...
    assert payload["holes"][0]["statement"] == 1
...
def test_coverage_holes_cli_routes_questa_statement_report(
    ...
    assert "Detailed coverage XML:" in output
