...
def test_parse_questa_fsm_coverage_normalizes_documented_state_transition_tables():
    ...
    transitions = [
        point for point in points if point["fsm_kind"] == "transition"
    ]

    assert states[0] == {
        "name": "/tb/dut|state|state:IDLE",
        "count": 7,
        "hit": True,
        "type": "fsm",
        "scope": "/tb/dut",
    ...
    assert transitions[0]["name"] == (
        "/tb/dut|state|transition:0 IDLE -> BUSY"
    )
...
def test_coverage_holes_cli_supports_questa_fsm_items(
    ...
    state_hole = next(
        hole for hole in payload["holes"] if hole["fsm_kind"] == "state"
    )
    assert state_hole["state"] == "ERROR"
    assert state_hole["line"] == 12
    transition_holes = [
        hole for hole in payload["holes"] if hole["fsm_kind"] == "transition"
    ]
    assert [hole["trans_id"] for hole in transition_holes] == [2, 3]
...
def test_coverage_cli_surfaces_questa_functional_snapshot(tmp_path: Path, monkeypatch, capsys):
    ...
    assert (
        "Normalized Questa FSM state/transition coverage: ok 7 point(s), 3 hole(s)"
        in output
    )
