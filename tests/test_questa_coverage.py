...
def test_merge_questa_coverage_keeps_other_evidence_when_code_xml_is_unavailable(
    tmp_path: Path,
    monkeypatch,
):
    project = _project(tmp_path)
...
def test_coverage_cli_surfaces_detailed_code_points(tmp_path: Path, monkeypatch, capsys):
    project = _project(tmp_path)
    monkeypatch.setattr("zddv.cli.load_project", lambda path: project)
    monkeypatch.setattr(
        "zddv.cli.merge_coverage",
    ...
    assert rc == 0
    output = capsys.readouterr().out
    assert "Detailed code coverage: xml (17 point(s))" in output
    assert "Normalized points: /tmp/points.json" in output
