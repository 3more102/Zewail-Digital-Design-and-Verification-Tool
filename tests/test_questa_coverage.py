...
def test_parse_questa_code_coverage_normalizes_documented_statement_and_branch_items():
...
def test_merge_questa_coverage_merges_reports_and_persists_snapshot(
    ...
    assert Path(result["code_report"]).read_text(
        encoding="utf-8"
    ) == QUESTA_CODE_DETAILS
...
def test_coverage_holes_cli_supports_questa_statement_and_branch_items(
    ...
    branch_hole = next(
        item for item in payload["holes"] if item["type"] == "branch"
    )
    assert branch_hole["source_file"] == "top.v"
    assert branch_hole["line"] == 12
    assert branch_hole["item"] == 1
