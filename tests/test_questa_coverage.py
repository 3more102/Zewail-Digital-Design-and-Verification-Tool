...
def test_coverage_holes_cli_combines_scalar_and_multibit_expression_fec(
    tmp_path: Path,
    monkeypatch,
    capsys,
):
    ...
    assert "[expression] ttest.sv:28:1:a[0]" in output

    payload = json.loads(
        (coverage_dir / "multibit-expression-holes.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    assert len(multibit) == 10
    target = next(hole for hole in multibit if hole["fec_target"] == "a[0]")
    assert target["bit"] == 0
    assert target["fec_hits"] == {"0": 1, "1": 0}
