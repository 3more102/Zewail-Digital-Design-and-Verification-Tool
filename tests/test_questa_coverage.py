...
""",
    ...
    assert points[0]["scope"] == "/tb/a"
    assert points[0]["source_file"] == "rtl/a.sv"
    assert points[1]["hit"] is True
    assert points[2]["item"] == 2


def test_write_questa_statement_holes_requests_detailed_by_instance_xml(
    tmp_path: Path,
    monkeypatch,
):
    project = _project(tmp_path)
...
""",
    ...
    assert report["holes"][0]["source_file"] == "rtl/dut.sv"
    assert report["holes"][0]["line"] == 20
    assert report["holes"][0]["item"] == 1
...
def test_coverage_holes_cli_routes_statement_to_by_instance_xml(
    ...
    assert "[statement] /tb/dut|rtl/dut.sv:20:1" in output
