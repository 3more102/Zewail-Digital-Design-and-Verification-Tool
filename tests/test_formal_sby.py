...
def test_render_sby_cover_config_uses_explicit_bound_and_project_sources(tmp_path: Path):
    project = _project(tmp_path)
    request = FormalCheckRequest(mode="cover", depth=24, timeout_s=1.2)
...
@pytest.mark.parametrize(
    ("formal_request", "message"),
    [
        (FormalCheckRequest(mode="cover"), "explicit depth"),
        (FormalCheckRequest(mode="bmc", depth=10), "cover mode only"),
        (
            FormalCheckRequest(mode="cover", depth=10, properties=("c_seen",)),
            "property filters",
        ),
    ],
)
def test_render_sby_cover_config_rejects_unsupported_requests(
    tmp_path: Path,
    formal_request: FormalCheckRequest,
    message: str,
):
    with pytest.raises(ValueError, match=message):
        render_sby_cover_config(_project(tmp_path), formal_request)
...
def test_parse_sby_status_jsonl_maps_cover_pass_fail_to_reachability():
    rows = parse_sby_status_jsonl(
        "\n".join(
            [
                '{"task_name":"demo","mode":"cover","name":"formal_top.c_seen","kind":"COVER","status":"PASS","depth":5,"trace":"/tmp/demo/engine_0/trace0.vcd"}',
                '{"task_name":"demo","mode":"cover","name":"formal_top.c_missing","kind":"COVER","status":"FAIL","depth":20}',
                '{"task_name":"demo","mode":"cover","name":"formal_top.a_safe","kind":"ASSERT","status":"PASS","depth":20}',
            ]
        )
        + "\n",
        mode="cover",
    ...
    assert [row.kind for row in rows] == ["cover", "cover"]
    assert [row.status for row in rows] == ["COVERED", "UNCOVERED"]
    assert [row.depth for row in rows] == [5, 20]
...
def test_sby_backend_ingests_cover_reachability_and_witnesses(
    tmp_path: Path,
    monkeypatch,
):
    project = _project(tmp_path)
        ...
        if "--statusfmt" in command:
            ...
            return SimpleNamespace(
                ...
                output=(
                    ...
                    '"depth":5,"trace":"/tmp/demo/engine_0/trace0.vcd"}\n'
                    '{"task_name":"demo","mode":"cover","engine":"smtbmc",'
    ...
    result = SymbiYosysBackend().check(
        project,
        FormalCheckRequest(mode="cover", depth=20, timeout_s=3.0),
    )
...
def test_formal_cover_cli_persists_reachability_history(tmp_path: Path, monkeypatch, capsys):
    project = _project(tmp_path)
            ...
            return FormalCheckResult(
                ...
                properties=(
                    FormalPropertyResult(
                        ...
                        status="COVERED",
                        depth=5,
                        trace_path=run_dir / "engine_0" / "trace0.vcd",
                    ),
                    FormalPropertyResult(
    ...
    assert rc == 1
    assert "Formal backend: sby test" in output
    assert "FORMAL COVER FAIL: depth=20 engine=smtbmc" in output
    assert "Coverage: covered=1 unreached=1" in output
    assert "Scope: COVER (finite-depth reachability evidence)" in output
