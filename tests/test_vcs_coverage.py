...
""",
    ...
    assert "Coverage holes (statement): 1 unhit point(s); 1 written" in printed
    assert "[statement] tb_top:line:14" in printed
...
def test_vcs_coverage_holes_rejects_unimplemented_metric(
    ...
    with pytest.raises(RuntimeError, match="supports --type statement"):
        cmd_coverage_holes(
            SimpleNamespace(
                project=str(project.root),
