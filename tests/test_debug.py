...
def test_assertion_waveform_fst_stays_metadata_only_without_explicit_converter(
    tmp_path: Path,
):
    project = initialize_project(tmp_path / "demo")
...
def test_assertion_waveform_cli_can_use_explicit_fst_converter(
    tmp_path: Path,
    capsys,
    monkeypatch,
):
    ...
    assert event_waveform["artifact"] == str(waveform.resolve())
